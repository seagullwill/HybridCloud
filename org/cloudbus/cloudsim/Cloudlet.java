...
public class Cloudlet {
	...
	/** The time where this Cloudlet is submitted. */
	...
	//private Map<Double, String> eventHistory=new HashMap<Double, String>();
	...
	public double getWaitingTime() {
		...
		final double subTime = getSubmitTime();
	...
	public void setResourceParameter(final int resourceID, final double cost) {
		...
		if (index == -1 && record) {
			...
			//		+ ") with cost = $" + cost + "/sec");
			//Log.printLine("Allocates this Cloudlet to " + res.resourceName + " (ID #" + resourceID
				//	+ ") with cost = $" + cost + "/sec");
		} else if (record) {
			...
			//Log.printLine("Moves Cloudlet from " + name + " (ID #" + id + ") to " + res.resourceName + " (ID #"
				//	+ resourceID + ") with cost = $" + cost + "/sec");
		}
	...
	//Log.printLine("resource中的提交时间："+clockTime+"index:"+index);
		final Resource res = resList.get(index);
		res.submissionTime = clockTime+ getSubmitTime();
	...
	public void setExecStartTime(final double clockTime) {
		...
		if (record) {//Log.printLine("Sets the execution start time to " + num.format(clockTime));
...
//	public Map<Double, String> getEventHistory() {		
	public List<String> getEventHistory(){
		return eventHistory;
	}

	
	//public void setEventHistory(Map<Double, String> eventHistory) {
	public void setEventHistory(List<String> eventHistory) {	
		this.eventHistory = eventHistory;
	}
	
	public void addEventHistory(double time, String status) {
		
		getEventHistory().add(time+","+ getCloudletId()+","+getVmId()+","+getClassType()+","+status);
		if(getEventHistory().size()>=100)
			saveEventHistory();		
	}
	
	public void saveEventHistory(){
		try
		{
			File csvRec = new File("D:\\data\\taskevent.csv");
			BufferedWriter bw = new BufferedWriter(new FileWriter(csvRec, true)); 
			/*Object[] keys=getEventHistory().keySet().toArray();
			Arrays.sort(keys);
			
			for(int i=0;i<100;i++){
				bw.newLine();
				bw.write(keys[i]+","+getEventHistory().get(keys[i]));
				getEventHistory().remove(keys[i]);	
			}*/
			for(String record:getEventHistory()){
				bw.newLine();
				bw.write(record);
